...
static void DecodeYoloGeneric(
    ...
    std::vector<int>& classIds)
{ 
	//return DecodeYoloGeneric_coco(out, yc, orig, padRect, boxesPx, scores, classIds);
    //return DecodeYoloGeneric_awz(out, yc, orig, padRect, boxesPx, scores, classIds);
    return DecodeYoloGeneric_y11(
        yolo_version,
        out, yc, orig, padRect, boxesPx, scores, classIds);
}
...
// cocoの640モデル、1280モデルで成功
// 独自モデルでは640、1280ともに成功 (/・ω・)/
static void DecodeYoloGeneric_y11(const cv::Mat& out, const YoloConfig& yc,
    const cv::Size& orig, const cv::Rect& padRect,
    std::vector<cv::Rect>& boxesPx,
    std::vector<float>& scores,
    std::vector<int>& classIds)
...
//#if(_YOLO11)
#if(1) //yolo5でも動く
    bool doSigmoidCls = false; // ダミーで一旦置き、後で再代入する（②で上書き）
    bool doSigmoidObj = false; // 同上
#else
    //const bool doSigmoid = yc.applySigmoid; // ONNXにSigmoidが残っていればfalse, ロジットならtrue

    bool doSigmoidCls = yc.applySigmoid; // ダミーで一旦置き、後で再代入する（②で上書き）
    bool doSigmoidObj = yc.applySigmoid; // 同上
#endif
...
//#if(_YOLO11) //yolo11判定
#if(1) //yolo5でも動く
    // ---- 出力が確率かロジットかを軽く自動判定（必要なら Sigmoid 適用）----
    auto peek_minmax = [&](int start, int count)->std::pair<float, float> {
        float mn = FLT_MAX, mx = -FLT_MAX;
        const int stride = std::max(1, N / 64);
        for (int i = 0, t = 0; i < N && t < 64; i += stride, ++t) {
    ...
    // まずはクラス側
    auto [mnCls, mxCls] = peek_minmax(clsStart, std::min(nc, 8)); // 8クラス分だけ覗く
    bool needSigmoidCls = (mnCls < -1e-3f) || (mxCls > 1.f + 1e-3f);
    ...
    // ★ 自動判定＋明示指定（applySigmoid）を反映して実使用フラグを確定
    doSigmoidCls = needSigmoidCls || yc.applySigmoid;
    doSigmoidObj = hasObj ? (needSigmoidObj || yc.applySigmoid) : false;
#else
#endif
...
//#if(!_YOLO11)
            ...
            if (doSigmoidCls)
                s = Sigmoid(s);
            const float conf = hasObj ? (s * obj) : s;

            if (nc <= 0 && (C > 64)) {
                // DFL(=reg_max*4=64) + nc と推定。ONNX側がデコード前出力の場合
                // → この場合の扱いはモデル依存なので、まずは安全に抜ける or ログ出力だけして抜ける
                // （将来対応するなら、ヘッド相当のDFLデコードをここで実装）
                return;
            }
#else
            //if (doSigmoid)
