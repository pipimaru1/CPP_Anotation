...
int LoadClassification(
    ...
    else if (_rw == 0) 
    {
		...
		//既存のobj.ClassNameのデータを更新
        for (size_t j = 0; j < GP.imgObjs.size(); ++j)
        {
            for (size_t k = 0; k < GP.imgObjs[j].objs.size(); ++k)
            {
				...
				{
					GP.imgObjs[j].objs[k].ClassName = _clsNames[_num];
					GP.imgObjs[j].objs[k].color = _clsColors[_num];
					GP.imgObjs[j].objs[k].dashStyle = _dashStyles[_num];
					GP.imgObjs[j].objs[k].penWidth = _clsPenWidths[_num];
				}
				else
				{
					GP.imgObjs[j].objs[k].ClassName = L"NO DATA";
					GP.imgObjs[j].objs[k].color = Gdiplus::Color(255, 255, 255);
					GP.imgObjs[j].objs[k].dashStyle = Gdiplus::DashStyleSolid;
					GP.imgObjs[j].objs[k].penWidth = 2;
				}
            }
        }
    }
